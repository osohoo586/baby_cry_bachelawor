...
# Эхний мөрийг хасах (толгой мөр)
metadata = metadata.iloc[1:].reset_index(drop=True)

# Анхны metadata-г харах
print("Анхны metadata-ийн эхний 5 мөр:")
print(metadata.head())

# Metadata-г цэвэрлэх
metadata['fold'] = metadata['fold'].str.replace('fold', '').astype(int)  # 'fold1' -> 1
metadata = metadata[metadata['file'].str.endswith('.wav')]  # Зөвхөн .wav файлууд
metadata = metadata.dropna(subset=['fold', 'file', 'label'])  # Хоосон утгыг хасах
metadata = metadata[metadata['fold'].isin([1, 2, 3, 4, 5])]  # Зөвхөн fold1-5
metadata['label'] = metadata['label'].str.strip().str.lower()  # Шошгыг цэвэрлэх

# Шошгыг бүрэн нэр болгох
label_map = {'bp': 'belly_pain', 'bu': 'burping', 'dc': 'discomfort', 'hu': 'hungry', 'ti': 'tired'}
metadata['label'] = metadata['label'].map(label_map)

print("Цэвэрлэгдсэн metadata-ийн эхний 5 мөр:")
print(metadata.head())

if metadata.empty:
    raise ValueError("Metadata хоосон байна! full_data.csv-г шалгаад зөв форматтай эсэхийг баталгаажуул.")
...
for index_num, row in tqdm(metadata.iterrows(), total=metadata.shape[0]):
    ...
    try:
        ...
        spectrogram = extract_spectrogram(audio, sr, target_width=128)
        ...
        if aug_factor > 0:
            for _ in range(aug_factor):
                ...
                spectrogram = extract_spectrogram(aug_audio, sr, target_width=128)
                data.append(spectrogram[..., np.newaxis])
                labels.append(label)
...
plt.plot(history.history['val_accuracy'], label='Баталгаажуулалтын нарийвчлал')
plt.title('Загварын нарийвчлал')
plt.xlabel('Эпох')
